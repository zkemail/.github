...
We have released a simple registry where you can define a new kind of proof in just a few minutes in the UI, and will automatically deploy infrastructure, a base repository, and on-chain verifiers for you, and then [expose an SDK](https://docs.zk.email/zk-email-sdk/setup) where you can use it from your frontend in just a few lines. We have [a beta version](https://registry.zk.email/) up for both client side and server side circom that is highly scalable when needed. We are adding features to the SDK and adding more prover backends (Noir, SP1) to make it easier to use. We hope that all proofs in the future go through this infrastructure due to its ease of use. Noir will be used for client side proofs, circom for server side proofs for wallets or small emails, and sp1 for server side proofs of large email bodies or attachments.
...
We've archived over a million DKIM public keys at [archive.prove.email](https://archive.prove.email) and read about how we did it (and recovered lost old keys) on [our blog post](https://zk.email/blog/archive)! We also expose a nice API for anyone to use to query this archive for selectors and public keys.
...
Q3 2024: Release a [registry for all existing ZK email proofs](https://registry.zk.email/) and regexes, and [an SDK](https://docs.zk.email/zk-email-sdk) by which to make new proofs or access existing proofs in a small handful of lines of Typescript [DONE]. Focus on auditing and scaling up our production deployments of account recovery [DONE], 2fa, and oauth. Finish [a Noir version](https://github.com/zkemail/zkemail.nr) [DONE]. Deploy account recovery to mainnet for Soul Wallet [DONE].

Q4 2024: Rewrite all of zk email in Noir [DONE](https://github.com/zkemail/zkemail.nr/), Rust [DONE](https://github.com/zkemail/zkemail.rs/), SP1 [DONE](https://github.com/zkemail/sp1-zkEmail), and Risc0 [DONE](https://github.com/risc0-labs/r0-zkEmail/), and natively integrate that into the registry. Deploy better, more scalable infra for the registry proofs [DONE](https://registry.zk.email). Continue to scale production level code, and add account recovery to more wallets [DONE, see Clave mainnet launch and Safe launch]. 

Q1 2025: Add hashed inputs to the registry [DONE], client side proving into the blueprint SDK [DONE], and improve bugs and scaling [DONE], and have at least one major launch with a partner here. Fix archive reverse engineering [DONE]. Finish ZK Regex Noir, and integrating Noir and SP1 into the registry. Work with Aztec and ZK Security to build a fuzzer into the regex implementation. Make Safe 2FA production ready by updating it to use our audited code and coordinating with the Safe team. Build a 7702 prototype for account recovery and try to get a PR into Porto.

Q2 2025: Add anonymous domain validation for full anonymity. Maybe add an on-chain dkim/public key/dns registry with an AVS? Add aggregation for recursive proofs with Axiom or Nebra?
...
| Title | Description |
|-------|-------------|
| **Build Elons Vision of X Payments** | Allow anyone to comment tip "tip $6 @tipbot", then turn on email notifications on @tipbot, then make a @zkemail proof that money was tipped, then transfer the money on chain and only allow the owner of that account to claim that money or move that money via Twitter comment. Claims can be via @zkemail proofs as well. More info at https://imgur.com/a/KIamUlB, from his biography. |
